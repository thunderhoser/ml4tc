...
More details on the input arguments are provided below.
 ...
 - `data_aug_max_translation_px`: Maximum translation (number of pixels) for satellite images.  Note that grid spacing is 4 km, *i.e.*, each pixel is 4 $\times$ 4 km.  The actual translation distance in both the $x$- and $y$-directions will be sampled from a uniform distribution over $\left[ -\textrm{data\_aug\_max\_translation\_px}, \textrm{data\_aug\_max\_translation\_px} \right]$ for each data sample.
 - `data_aug_num_rotations`: Number of random rotations to apply to each data sample (affects satellite images only).
 - `data_aug_max_rotation_deg`: Maximum rotation (degrees) for satellite images.  The actual rotation angle will be sampled from a uniform distribution over $\left[ -\textrm{data\_aug\_num\_rotations}, \textrm{data\_aug\_num\_rotations} \right]$ for each data sample.
