"""Plots eval metrics vs. hyperparams for MCd Experiment 1-ext for Taiwan."""
...
SECOND_LAST_LAYER_DROPOUT_RATES = numpy.linspace(0.5, 0.9, num=5, dtype=float)
THIRD_LAST_LAYER_DROPOUT_RATES = numpy.linspace(0.5, 0.9, num=5, dtype=float)
FOURTH_LAST_LAYER_DROPOUT_RATES = numpy.linspace(0.5, 0.9, num=5, dtype=float)
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    :param score_matrix: X-by-Y-by-Z-by-L numpy array of scores.
...
def _print_ranking_all_scores(
    ...
    :param auc_matrix: X-by-Y-by-Z-by-L numpy array with AUC (area under ROC
        curve).
    ...
    for m in range(len(LEAD_TIMES_HOURS)):
        ...
        for a in range(len(i_sort_indices)):
            ...
            print((
                ...
                'SSREL rank = {10:.1f} ... MF rank = {11:.1f} ... '
                'predictive-stdev rank = {12:.1f}'
            ).format(
...
def _run(experiment_dir_name, top_output_dir_name):
    ...
    dimensions = (
        ...
        num_second_last_dropout_rates, num_lead_times
    )
            ...
            for k in range(num_second_last_dropout_rates):
                for m in range(num_lead_times):
                    this_score_file_name = (
                        ...
                        'validation_sans_uq/{4:s}/evaluation.nc'
                    ).format(
                        experiment_dir_name,
                        FOURTH_LAST_LAYER_DROPOUT_RATES[i],
                        THIRD_LAST_LAYER_DROPOUT_RATES[j],
                    ...
                    auc_matrix[i, j, k, m] = numpy.mean(
                        t[evaluation.AUC_KEY].values
                    )
                    aupd_matrix[i, j, k, m] = numpy.mean(
                        t[evaluation.AUPD_KEY].values
                    )
                    bss_matrix[i, j, k, m] = numpy.mean(
                    ...
                    frequency_bias_matrix[i, j, k, m] = numpy.mean(
                        t[evaluation.FREQUENCY_BIAS_KEY].values[this_index, :]
                    )

                    this_score_file_name = (
                        '{0:s}/dropout-rates={1:.3f}-{2:.3f}-{3:.3f}/'
                        'validation_with_uq/{4:s}/spread_vs_skill.nc'
                    ).format(
                        experiment_dir_name,
                        FOURTH_LAST_LAYER_DROPOUT_RATES[i],
                        THIRD_LAST_LAYER_DROPOUT_RATES[j],
                    ...
                    ssrel_matrix[i, j, k, m] = result_dict[
                    ...
                    mean_predictive_stdev_matrix[i, j, k, m] = numpy.average(
                    ...
                    this_score_file_name = (
                        '{0:s}/dropout-rates={1:.3f}-{2:.3f}-{3:.3f}/'
                        'validation_with_uq/{4:s}/discard_test.nc'
                    ).format(
                        experiment_dir_name,
                        FOURTH_LAST_LAYER_DROPOUT_RATES[i],
                        THIRD_LAST_LAYER_DROPOUT_RATES[j],
                    ...
                    monotonicity_fraction_matrix[i, j, k, m] = (
    ...
    for m in range(num_lead_times):
        ...
        for k in range(num_second_last_dropout_rates):
            ...
            figure_object, axes_object = _plot_scores_2d(
                ...
                max_colour_value=numpy.nanpercentile(auc_matrix[..., m], 99),
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
                colour_map_object=DEFAULT_COLOUR_MAP_OBJECT
            )

            title_string = (
                'AUC for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
            )
            ...
            # Plot AUPD.
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=aupd_matrix[..., k, m],
                min_colour_value=numpy.nanpercentile(aupd_matrix[..., m], 1),
                max_colour_value=numpy.nanpercentile(aupd_matrix[..., m], 99),
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
                colour_map_object=DEFAULT_COLOUR_MAP_OBJECT
            )

            title_string = (
                'AUPD for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
            ...
            title_string = (
                'BSS for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
            ...
            # Plot CSI.
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=csi_matrix[..., k, m],
                min_colour_value=numpy.nanpercentile(csi_matrix[..., m], 1),
                max_colour_value=numpy.nanpercentile(csi_matrix[..., m], 99),
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
                colour_map_object=DEFAULT_COLOUR_MAP_OBJECT
            )

            title_string = (
                'CSI for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
            ...
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=frequency_bias_matrix[..., k, m],
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
            ...
            title_string = (
                'Freq bias for {0:s}, dropout rate = {1:.3f} in second-last '
                'layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
            ...
            # Plot SSREL.
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=ssrel_matrix[..., k, m],
                min_colour_value=numpy.nanpercentile(ssrel_matrix[..., m], 1),
                max_colour_value=numpy.nanpercentile(ssrel_matrix[..., m], 99),
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
                colour_map_object=DEFAULT_COLOUR_MAP_OBJECT
            )

            title_string = (
                'SSREL for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
            ...
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=mean_predictive_stdev_matrix[..., k, m],
                min_colour_value=
                numpy.nanpercentile(mean_predictive_stdev_matrix[..., m], 1),
                max_colour_value=
                numpy.nanpercentile(mean_predictive_stdev_matrix[..., m], 99),
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
                colour_map_object=DEFAULT_COLOUR_MAP_OBJECT
            )

            title_string = (
                'Stdev for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
            ...
            figure_object, axes_object = _plot_scores_2d(
                score_matrix=monotonicity_fraction_matrix[..., k, m],
                min_colour_value=
                numpy.nanpercentile(monotonicity_fraction_matrix[..., m], 1),
                max_colour_value=
                numpy.nanpercentile(monotonicity_fraction_matrix[..., m], 99),
                x_tick_labels=x_tick_labels, y_tick_labels=y_tick_labels,
                colour_map_object=DEFAULT_COLOUR_MAP_OBJECT
            )

            title_string = (
                'MF for {0:s}, dropout rate = {1:.3f} in second-last layer'
            ).format(
                'all lead times' if LEAD_TIMES_HOURS[m] < 0
                else '{0:d}-hour lead time'.format(LEAD_TIMES_HOURS[m]),
                SECOND_LAST_LAYER_DROPOUT_RATES[k]
