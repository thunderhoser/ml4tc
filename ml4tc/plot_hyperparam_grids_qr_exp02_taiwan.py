"""Plots evaluation metrics vs. hyperparams for QR Experiment 2 for Taiwan."""
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    for j in range(len(LEAD_TIMES_HOURS)):
        these_scores = score_matrix[:, j] + 0.
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        sort_indices = numpy.argsort(-these_scores)
        
        for i in range(len(sort_indices)):
            print((
                '{0:d}th-highest {1:s} at {2:d}-hour lead time = {3:.4g} ... '
                'central-loss weight = {4:.1f}'
            ).format(
                i, score_name, LEAD_TIMES_HOURS[j],
                score_matrix[sort_indices[i], j],
...
def _print_ranking_all_scores(
    ...
    :param auc_matrix: W-by-L numpy array with AUC (area under ROC curve).
    ...
    for j in range(len(LEAD_TIMES_HOURS)):
        if rank_mainly_by_auc:
            these_scores = -1 * auc_matrix[:, j]
            these_scores[numpy.isnan(these_scores)] = -numpy.inf
        else:
            these_scores = ssrel_matrix[:, j] + 0.
            these_scores[numpy.isnan(these_scores)] = numpy.inf
        ...
        these_scores = -1 * auc_matrix[:, j]
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        auc_ranks = rankdata(these_scores, method='average')
    
        these_scores = -1 * aupd_matrix[:, j]
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        aupd_ranks = rankdata(these_scores, method='average')

        these_scores = -1 * bss_matrix[:, j]
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        bss_ranks = rankdata(these_scores, method='average')

        these_scores = -1 * csi_matrix[:, j]
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        csi_ranks = rankdata(these_scores, method='average')
        
        these_scores = numpy.absolute(1. - frequency_bias_matrix[:, j])
        these_scores[numpy.isnan(these_scores)] = numpy.inf
        frequency_bias_ranks = rankdata(these_scores, method='average')

        these_scores = ssrel_matrix[:, j] + 0.
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        ssrel_ranks = rankdata(these_scores, method='average')

        these_scores = -1 * mean_predictive_stdev_matrix[:, j]
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        mean_predictive_stdev_ranks = rankdata(these_scores, method='average')

        these_scores = -1 * monotonicity_fraction_matrix[:, j]
        these_scores[numpy.isnan(these_scores)] = -numpy.inf
        monotonicity_fraction_ranks = rankdata(these_scores, method='average')
        ...
        for i in range(len(sort_indices)):
            ...
            print((
                ...
                i + 1, LEAD_TIMES_HOURS[j], CENTRAL_LOSS_WEIGHTS[k],
                auc_ranks[k], aupd_ranks[k], bss_ranks[k], csi_ranks[k],
...
def _run(experiment_dir_name, output_dir_name):
    ...
    dimensions = (num_central_loss_weights, num_lead_times)
    ...
    for i in range(num_central_loss_weights):
        for j in range(num_lead_times):
            ...
            this_score_file_name = (
                '{0:s}/central-loss-weight={1:04.1f}/validation_with_uq/{2:s}/'
                'spread_vs_skill.nc'
            ).format(
                experiment_dir_name, CENTRAL_LOSS_WEIGHTS[i],
                'all_lead_times' if LEAD_TIMES_HOURS[j] < 0
