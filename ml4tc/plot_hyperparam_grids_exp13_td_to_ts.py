"""Plots scores on hyperparam grid for Experiment 13, TD-to-TS prediction."""
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    D = number of data-augmentation flags

    :param score_matrix: P-by-L-by-D numpy array of scores.
...
def _add_markers(figure_object, axes_object, best_marker_indices):
    ...
    marker_size_px = figure_width_px * (
        BEST_MARKER_SIZE_GRID_CELLS / len(LAG_TIME_COUNTS)
    )
    axes_object.plot(
    ...
    marker_size_px = figure_width_px * (
        SELECTED_MARKER_SIZE_GRID_CELLS / len(LAG_TIME_COUNTS)
    )
    axes_object.plot(
...
def _run(experiment_dir_name, output_dir_name):
    ...
    num_predictor_sets = len(LAGGED_PREDICTOR_GROUP_STRINGS)
    num_lag_time_sets = len(LAG_TIME_COUNTS)
    num_data_aug_flags = len(USE_DATA_AUG_FLAGS)
    dimensions = (num_predictor_sets, num_lag_time_sets, num_data_aug_flags)
    ...
    for i in range(num_predictor_sets):
        for j in range(num_lag_time_sets):
            for k in range(num_data_aug_flags):
                this_score_file_name = (
                    '{0:s}/num-lag-times={1:02d}_lagged-ships-predictors={2:s}_'
    ...
    for k in range(num_data_aug_flags):
        ...
        axes_object.set_title(
            'Area under ROC curve {0:s} data augmentation'.format(
                'with' if USE_DATA_AUG_FLAGS[k] else 'without'
            )
        )

        output_file_name = '{0:s}/auc_data-aug={1:d}.jpg'.format(
            output_dir_name, int(USE_DATA_AUG_FLAGS[k])
        )
        ...
        axes_object.set_ylabel(y_axis_label)
        axes_object.set_title(
            'Area under perf diagram {0:s} data augmentation'.format(
                'with' if USE_DATA_AUG_FLAGS[k] else 'without'
            )
        )

        output_file_name = '{0:s}/aupd_data-aug={1:d}.jpg'.format(
            output_dir_name, int(USE_DATA_AUG_FLAGS[k])
        )
        print('Saving figure to: "{0:s}"...'.format(output_file_name))
        figure_object.savefig(
        ...
        axes_object.set_ylabel(y_axis_label)
        axes_object.set_title(
            'Brier skill score {0:s} data augmentation'.format(
                'with' if USE_DATA_AUG_FLAGS[k] else 'without'
            )
        )

        output_file_name = '{0:s}/bss_data-aug={1:d}.jpg'.format(
            output_dir_name, int(USE_DATA_AUG_FLAGS[k])
        )
        print('Saving figure to: "{0:s}"...'.format(output_file_name))
        figure_object.savefig(
        ...
        axes_object.set_ylabel(y_axis_label)
        axes_object.set_title(
            'Critical success index {0:s} data augmentation'.format(
                'with' if USE_DATA_AUG_FLAGS[k] else 'without'
            )
        )

        output_file_name = '{0:s}/csi_data-aug={1:d}.jpg'.format(
            output_dir_name, int(USE_DATA_AUG_FLAGS[k])
        )
        print('Saving figure to: "{0:s}"...'.format(output_file_name))
        figure_object.savefig(
        ...
        axes_object.set_ylabel(y_axis_label)
        axes_object.set_title(
            'Frequency bias {0:s} data augmentation'.format(
                'with' if USE_DATA_AUG_FLAGS[k] else 'without'
            )
        )

        output_file_name = '{0:s}/frequency_bias_data-aug={1:d}.jpg'.format(
            output_dir_name, int(USE_DATA_AUG_FLAGS[k])
        )
        print('Saving figure to: "{0:s}"...'.format(output_file_name))
        figure_object.savefig(
