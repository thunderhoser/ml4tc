"""Makes CNN templates for BNN Experiment 2 for rapid intensification."""
...
KL_SCALING_FACTORS = numpy.logspace(-8.5, -5.5, num=7)
...
OUTPUT_DIR_NAME = (
    ...
    'bnn_experiment02_scaling/templates'
)
...
def _run():
    ...
    for i in range(num_layer_type_sets):
        for j in range(num_kl_scaling_factors):
            ...
            option_dict_dense[bcnn_architecture.KL_SCALING_FACTOR_KEY] = (
                KL_SCALING_FACTORS[j]
            )
            ...
            model_file_name = (
                '{0:s}/layer-types={1:s}_kl-scaling-factor={2:.12f}/model.h5'
            ).format(
                OUTPUT_DIR_NAME, LAYER_TYPE_ABBREVS[i], KL_SCALING_FACTORS[j]
            )
