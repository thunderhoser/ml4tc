"""Plots evaluation scores on hyperparameter grid for Experiment 14."""
...
USE_DATA_AUG_FLAGS = numpy.array([0, 1], dtype=int)
...
def _print_ranking_one_score(score_matrix, score_name):
    ...
    :param score_matrix: D-by-L numpy array of scores.
    ...
    for k in range(len(i_sort_indices)):
        ...
        print((
            '{0:d}th-highest {1:s} = {2:.4g} ... data augmentation = {3:d} ... '
            'lag times = {4:s} h'
        ).format(
            k + 1, score_name, score_matrix[i, j],
            USE_DATA_AUG_FLAGS[i], LAG_TIME_STRINGS_HOURS[j]
        ))
...
def _run(experiment_dir_name, output_dir_name):
    ...
    dimensions = (num_data_aug_flags, num_lag_time_sets)
    ...
    y_tick_labels = [
        '{0:s}'.format('Yes' if f == 1 else 'No') for f in USE_DATA_AUG_FLAGS
    ]
    x_tick_labels = ['{0:s}'.format(s) for s in LAG_TIME_STRINGS_HOURS]
    y_axis_label = 'Data augmentation?'
    x_axis_label = 'Lag times (hours)'

    for i in range(num_data_aug_flags):
        for j in range(num_lag_time_sets):
            this_score_file_name = (
                '{0:s}/lag-times-minutes={1:s}_use-data-aug={2:d}/'
                'validation_2005-2009/evaluation.nc'
            ).format(
                experiment_dir_name, LAG_TIME_STRINGS_MINUTES[j],
                USE_DATA_AUG_FLAGS[i]
            )
